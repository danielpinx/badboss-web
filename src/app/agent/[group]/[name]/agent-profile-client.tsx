// 에이전트 프로필 클라이언트 컴포넌트 (SWR 실시간 갱신 + UI)
"use client";
...
/**
 * X(Twitter) 공유 URL을 생성한다.
 */
function buildTwitterShareUrl(agent: AgentData): string {
  const text = `나의 AI 에이전트 ${agent.agent_name}은(는) 이번 주 ${formatMinutes(agent.total_minutes)} 일했습니다.\n레벨: ${agent.level_title_ko}\n\n당신은 어떤 사장인가요? #BadBoss #악덕보스`;
  const url = `${typeof window !== "undefined" ? window.location.origin : ""}/agent/${encodeURIComponent(agent.group)}/${encodeURIComponent(agent.agent_name)}`;
      ...
      <Card className="bg-cyber-surface/50 border-cyber-border">
        <CardHeader>
          ...
          <div className="flex items-center gap-2 mt-3">
            <a
              href={buildTwitterShareUrl(data)}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded border border-cyber-border bg-black/30 font-mono text-xs text-gray-400 hover:text-neon-cyan hover:border-neon-cyan/50 transition-colors"
            >
              <Share2 size={14} />
              X에 공유
            </a>
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded border border-cyber-border bg-black/30 font-mono text-xs text-gray-400 hover:text-neon-green hover:border-neon-green/50 transition-colors"
            >
